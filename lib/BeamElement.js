...
const BeamJava = (() => {
  ...
  const VertexConsumerProviderImmediate = 'net.minecraft.class_4597$class_4598'
  ...
  Reflection.createClassBuilder(className, Java.type(Line3D))
    .addField(`public static ${VertexConsumerProviderImmediate} vcp;`)
    ...
    .addField(`public ${Identifier} texture = ${BeaconBlockEntityRenderer}.${BEAM_TEXTURE};`)
    ...
    .addMethod(`public void render(${DrawContext} drawContext, ${BufferBuilder} builder, float tickDelta) {
      ...
      float delta = 0;
      if (speed != 0.0f) {
        delta = (float) Math.floorMod((long) mc.${world}.${getTime}(), (int) (Math.abs(speed) < 1 ? 40000 : 40)) + tickDelta;
        if (speed != 1.0f) {
          delta *= speed;
          delta %= 40;
          if (delta < 0) delta += 40;
...
const entityVertexConsumers = bufferBuildersF.get(Client.getMinecraft().field_1769).method_23000()
BeamJava.vcp = entityVertexConsumers
